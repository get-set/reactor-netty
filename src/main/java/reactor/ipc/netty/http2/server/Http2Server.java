...
public abstract class Http2Server {
	...
	public final Http2Server handler(BiFunction<? super Http2StreamInbound, ? super Http2StreamOutbound, ? extends Publisher<Void>> handler) {
		...
		return tcpConfiguration(tcp -> tcp.doOnConnection(c -> {
			if (c.channel().pipeline().get(Http2ServerHandler.class) != null) {
				((Http2ServerOperations) c).streams().subscribe(s -> {
					if (log.isDebugEnabled()) {
						log.debug("{} HTTP/2 handler is being applied: {}", c.channel(), handler);
					}

					Mono.fromDirect(handler.apply((Http2StreamInbound) c, (Http2StreamOutbound) c))
	...
	static final ChannelOperations.OnSetup HTTP2_OPS = new ChannelOperations.OnSetup() {
		@Nullable
		@Override
		public ChannelOperations<?, ?> create(Connection c, ConnectionEvents listener, Object msg, Integer streamId) {
			return Http2ServerOperations.bindHttp2(c, listener, msg, streamId);
		}
